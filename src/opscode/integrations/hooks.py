"""Event hook dispatch for opscode.
...
def _default_hooks_path() -> Path:
    """Return ``~/.opscode/hooks.json``."""
    from opscode.config.paths import HOOKS_PATH
    return HOOKS_PATH
...
def load_hooks(path: Path | None = None) -> list[HookConfig]:
    ...
    Args:
        path: Explicit path.  Falls back to ``~/.opscode/hooks.json``.
...
async def _run_hook(hook: HookConfig, event: str, payload: dict[str, Any]) -> None:
    ...
    env = {**os.environ, "OPSCODE_HOOK_EVENT": event}
