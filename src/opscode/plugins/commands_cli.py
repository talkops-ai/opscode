"""CLI helpers for plugin management in OpsCode."""
...
from opscode.plugins.models import InstallScope
from opscode.plugins.store import load_marketplace_records
...
def _resolve_project_root(scope: str | None) -> Path | None:
    ...
    if scope in {None, "user"}:
        return None
    from opscode.config.settings import _find_project_root  # noqa: PLC2701
...
def setup_plugin_parser(
    ...
    *,
    ...
    def _default_help() -> None:
        print("Usage: opscode plugin {list,install,uninstall,enable,disable,marketplace}")
...
def execute_plugin_command(args: argparse.Namespace) -> str | None:
    ...
    if command is None:
        text = "Usage: opscode plugin {list,install,uninstall,enable,disable,marketplace}"
        print(text)
        return text
