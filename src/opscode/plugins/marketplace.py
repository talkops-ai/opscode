"""Marketplace parsing for OpsCode plugins."""
...
from opscode.plugins._json import json_object
from opscode.plugins.manifest import _resolve_component_path, _validate_name
...
_MARKETPLACE_RELATIVE_PATHS = (
    ...
    Path(".opscode-plugin") / "marketplace.json",
    Path(".opscode") / ".opscode-plugin" / "marketplace.json",
    Path(".opscode") / "plugins" / "marketplace.json",
...
def _download_marketplace(url: str) -> Path:
    ...
    request = urllib.request.Request(
        url, headers={"User-Agent": "opscode-plugin-manager"}
    )
