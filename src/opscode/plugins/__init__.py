"""OpsCode Plugin System.

Provides plugin discovery, marketplace management, and plugin protocol
for extending OpsCode with new skills, MCP servers, and capabilities.
"""
...
from opscode.plugins.discovery import (
...
from opscode.plugins.models import PluginDiscoveryResult, PluginInstance
from opscode.ui.command_registry import SlashCommand
...
def discover_plugins() -> list[OpsCodePlugin]:
    """Discover and return all active Python plugins (built-in + entrypoints)."""
    plugins: list[OpsCodePlugin] = [TerraformPlugin()]
    try:
        eps = importlib.metadata.entry_points(group="opscode.plugins")
        for ep in eps:
            try:
                plugin_cls = ep.load()
...
from opscode.plugins.commands_cli import (
