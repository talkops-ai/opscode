...
When a project contains a marketplace manifest inside its ``.opscode`` directory
(e.g. ``.opscode/.opscode-plugin/marketplace.json``), this module automatically
...
if TYPE_CHECKING:
    from opscode.plugins.adapters.skills import PluginSkillSource
...
def _build_plugin_instance(
    ...
    try:
        from opscode.plugins.store import plugin_data_dir
...
def _process_main_plugin(
    ...
    """
    from opscode.plugins.adapters.skills import plugin_skill_sources
...
def load_project_plugins(project_root: Path | None) -> ProjectPluginResult:
    """Auto-discover and load all plugins from a project's ``.opscode`` marketplace.

    This is the main entry point called by ``factory.py`` during agent assembly.

    Args:
        project_root: Path to the project root (contains ``.opscode/``).
    ...
    # Look for a marketplace manifest inside the .opscode directory
    opscode_dir = project_root / ".opscode"
    if not opscode_dir.is_dir():
        return result

    manifest_path = find_marketplace_manifest(opscode_dir)
    ...
    from opscode.plugins.store import load_all_disabled_plugin_ids
