...
from opscode.config.paths import CONFIG_PATH as DEFAULT_CONFIG_PATH
...
def apply_stored_credentials(provider: str) -> bool:
    """Export stored provider API key and env vars from ~/.opscode/.env to os.environ.
    ...
    """
    from opscode.config.settings import _load_dotenv, resolve_env_var
...
def revoke_provider_credentials(provider: str | None = None, settings: Any | None = None) -> list[str]:
    ...
    credential lines from ~/.opscode/.env file.

    Returns a list of provider names whose credentials were revoked.
    """
    from opscode.config.paths import DATA_DIR, GLOBAL_ENV_PATH
...
from opscode.config.paths import STATE_DIR as _STATE_DIR, RECENT_MODELS_PATH as _RECENT_MODELS_FILE
