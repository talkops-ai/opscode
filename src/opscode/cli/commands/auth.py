...
from opscode.config.paths import GLOBAL_ENV_PATH, upsert_env_vars
from opscode.model.config import get_credential_env_var, get_provider_display_name, has_provider_credentials
...
def setup_auth_parser(
    ...
    *,
    ...
    """Register the `opscode auth` command group."""
...
def run_auth_command(args: argparse.Namespace) -> int:
    """Dispatch a parsed `opscode auth` invocation."""
    ...
    from opscode.ui.ui_help import show_auth_help
    ...
    if from_env:
        ...
        if sys.stdin.isatty():
            console.print(
                ...
                f"Pipe via stdin (`echo 'KEY' | opscode auth set {provider}`) or use `--from-env VAR`."
            )
            return 2
