"""CLI commands for inspecting and editing OpsCode configuration."""
...
from opscode.config.paths import CONFIG_PATH, DATA_DIR, GLOBAL_ENV_PATH, STATE_DIR
from opscode.config.settings import settings
from opscode.config.toml_config import _save_toml_field, read_config_toml
...
def setup_config_parser(
    ...
    *,
    ...
    """Register the `opscode config` command group."""
...
def run_config_command(args: argparse.Namespace) -> int:
    """Dispatch `opscode config` commands."""
...
def _collect_config_items() -> list[dict[str, Any]]:
    ...
    items: list[dict[str, Any]] = [
        ...
        {
            "key": "general.theme",
            "value": toml_data.get("general", {}).get("theme", "opscode-dark"),
...
def _run_summary(*, verbose: bool = False, output_format: OutputFormat = "text") -> int:
    ...
    console = Console()
    table = Table(title="OpsCode Configuration", show_header=True, header_style=f"bold {DC_TEAL}")
