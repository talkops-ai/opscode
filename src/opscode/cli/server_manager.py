"""Server lifecycle orchestration for opscode.
...
from opscode.server.server import _EPHEMERAL_PORT, ServerProcess
...
_DISTRIBUTION_NAME = "opscode"
_SERVER_ENV_PREFIX = "OPSCODE_SERVER_"
...
def _write_checkpointer(work_dir: Path) -> None:
    from opscode.config.paths import SESSIONS_DB_PATH
...
async def create_checkpointer() -> AsyncGenerator[Any, None]:
    ...
    if not db_path:
        raise RuntimeError(
            "{db_path_env} not set. The opscode CLI must set this "
...
async def start_server_and_get_agent(
    *,
    ...
    work_dir = Path.home() / ".opscode" / ".state" / "server"
    ...
    from opscode.ui.remote_client import RemoteAgent
...
async def server_session(
    *,
    ...
    Usage::
        ...
        async with server_session(assistant_id="opscode") as (client, server):
