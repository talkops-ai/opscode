"""Interactive TUI runner for opscode.

Launches the Textual ``OpsCodeApp`` immediately and starts the LangGraph dev
...
def run_interactive(
    *,
    ...
    from opscode.exceptions import NoCredentialsConfiguredError
    from opscode.model.config import apply_stored_credentials
    from opscode.model.factory import _get_default_model_spec, detect_provider, normalize_model_spec
    ...
    async def _run() -> int:
        ...
        if thread_id and return_code == 0:
            try:
                ...
                hint = Text("opscode -r ", style="cyan")
