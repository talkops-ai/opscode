...
from opscode.config.settings import get_glyphs
...
class TextualAdapter:
    ...
    # directly from the stream loop to avoid flooding the event queue.
    ...
    def set_client(self, client: Any) -> None:
        ...
        Called by ``OpsCodeApp`` when the background server worker posts
    ...
    def _prepare_stream_input(
        ...
        from opscode.middleware.auto_mode import USER_PROMPT_METADATA_KEY, user_prompt_metadata

        if graph_input is not None:
            return graph_input, None

        from opscode.input import parse_file_mentions
        ...
        if self._app is not None:
            from opscode.commands.power.goal import get_goal_state, GoalHandler
    ...
    async def _prepare_stream_config(
        ...
        """
        from opscode.approval_mode import ApprovalMode, awrite_approval_mode, coerce_approval_mode
        from opscode.config.metadata import build_stream_config
    ...
    def _handle_custom_stream_event(self, data: Any, is_main_agent: bool) -> None:
        ...
        if self._on_subagent_event is not None and _is_renderable_subagent_event(data, is_main_agent=is_main_agent):
            ...
            if formatted_event is not None and is_main_agent and self._messages is not None:
                ...
                from opscode.ui.widgets.messages import RubricResultMessage, SystemMessage, ErrorMessage
    ...
    def _finalize_turn_error(self, error: Exception) -> None:
        ...
        if self._messages is not None:
            self._messages.finish_assistant_message()
            from opscode.ui.widgets.messages import ErrorMessage
