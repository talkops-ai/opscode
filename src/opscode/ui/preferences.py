...
Parses and writes `~/.opscode/config.toml` for UI theme, scrollbar, timestamps,
...
def load_preferences() -> UserPreferences:
    """Load preferences from ~/.opscode/config.toml if present."""
...
def save_preferences(prefs: UserPreferences) -> bool:
    """Save preferences to ~/.opscode/config.toml."""
...
class ThemeSelector(Widget):
    ...
    def action_apply(self) -> None:
        ...
        if opt_list.highlighted is not None:
            ...
            theme_name = option.id or "opscode-dark"
