"""OpsCode Textual TUI application orchestrator.
...
from opscode.ui.widgets.goal_review import GoalReviewMenu
from opscode.ui.widgets.autocomplete import AutocompletePopup
from opscode.ui.widgets.chat_input import ChatInput, InputMode
...
from opscode.ui.widgets.toast import show_toast
from opscode.ui.widgets.welcome import WelcomeBanner

from opscode.ui.permission_store import PermissionStore, load_permission_store
from opscode.utils.git import (
...
class OpsCodeApp(App):
    """OpsCode interactive TUI — DevOps coding agent orchestrator."""
    ...
    def __init__(
        ...
        *,
        ...
        if not model and not defer_server_start:
            try:
                from opscode.model.factory import _get_default_model_spec
        ...
        # ── Command Router ────────────────────────────────
        from opscode.commands import CommandRouter
        self._command_router = CommandRouter()
        self._command_router.auto_discover()

    def _init_themes(self) -> None:
        """Register OpsCode brand and custom themes and apply initial preference."""
        try:
            register_app_themes(self)
            initial_theme = load_theme_preference()
            self.theme = initial_theme if initial_theme in get_registry() else "opscode-dark"
        except Exception:
    ...
    def get_theme_variable_defaults(self) -> dict[str, str]:
        ...
        """
        from opscode.ui.theme import get_css_variable_defaults, get_theme_colors
    ...
    def compose(self) -> ComposeResult:
        from opscode.ui.widgets.goal_status import GoalStatusPanel
        from opscode.ui.widgets.subagent_panel import SubagentPanel
    ...
    async def _set_spinner(self, status: str | None) -> None:
        """Show, update, or hide the loading spinner in the message list."""
    ...
    async def maybe_start_deferred_server(self) -> bool:
        ...
        from opscode.exceptions import ModelConfigError, NoCredentialsConfiguredError
        from opscode.model.factory import _get_default_model_spec
    ...
    def on_mouse_up(self, event: events.MouseUp) -> None:
        ...
        from opscode.ui.clipboard import copy_selection_to_clipboard
    ...
    async def _cleanup_agent_task(
        ...
        *,
        ...
        try:
            ...
            # Regroup completed tools at turn boundary
            try:
    ...
    #
    # Goal infrastructure: mutation boundary, checkpoint persistence, state
    # sync from thread, criteria request submission, and accept flow.

    @asynccontextmanager
    async def _goal_state_mutation_boundary(self) -> AsyncGenerator[None, None]:
    ...
    async def _get_thread_state_values(self, thread_id: str | None = None) -> dict[str, Any]:
        ...
        table through LangChain's ``add_messages`` reducer. The fallback
        filters ``checkpoint_ns = ''`` (parent graph only) so sub-graph messages never leak.
        ...
        # add_messages reducer (with checkpoint_ns='').
    ...
    async def _reconstruct_state_from_local_db(
        ...
        filtered to ``checkpoint_ns = ''`` (parent graph only) — and replays them through
    ...
    async def _continue_goal_work(
        ...
        *,
        ...
        """
        from opscode.middleware.goal_state_notice import build_goal_continuation
    ...
    async def _handle_command(self, text: str) -> None:
        ...
        if handler is not None:
            ...
            from opscode.commands import CommandContext
            from opscode.config.settings import settings as global_settings
            ctx = CommandContext(
                app=self,
    ...
    async def _invoke_reload(self, *, command: str | None = None) -> None:
        ...
        try:
            from opscode.ui.theme import register_app_themes
            register_app_themes(self)
        except Exception as exc:
    ...
    async def _handle_install_command(self, command: str) -> None:
        ...
        if installer == "uv" and not is_pkg:
            cmd_args = ["uv", "tool", "install", "--reinstall", "-U", f"opscode[{target}]"]
    ...
    async def _handle_update_command(self, command: str = "/update") -> None:
        """Check for and apply OpsCode software updates."""
        await self._mount_message(UserMessage(command))
        parts = command.strip().split()
        ...
        try:
            from opscode import __version__ as cli_version
        except ImportError:
            cli_version = "unknown"

        await self._mount_message(SystemMessage(f"Checking updates for OpsCode (current: v{cli_version})..."))
        from opscode.commands.power.runtime import _check_pypi_version, _perform_upgrade
        ...
        if latest == cli_version:
            await self._mount_message(
                SystemMessage(f"OpsCode v{cli_version} is on the latest version.")
            )
            return

        await self._mount_message(SystemMessage(f"Updating OpsCode v{cli_version} → v{latest}..."))
        success, output = await asyncio.to_thread(_perform_upgrade, target_version=latest, prerelease=prerelease)
        if success:
            await self._mount_message(SystemMessage(f"✅ Updated OpsCode to v{latest}. Run `/restart` to apply."))
        else:
            await self._mount_message(ErrorMessage(f"Failed to update OpsCode:\n{output}"))
    ...
    async def _load_thread_history(self, thread_id: str) -> None:
        ...
        for idx, msg in enumerate(raw_messages):
            ...
            if isinstance(msg, HumanMessage) or msg_type == "human":
                ...
                from opscode.ui.textual_adapter import _extract_text_and_thinking
        ...
        from opscode.utils.cost_estimation import estimate_cost
    ...
    async def _has_conversation_messages(self) -> bool:
        """Check if the active session has conversation messages."""
        try:
            from opscode.ui.widgets.messages import UserMessage, AssistantMessage, MessageList
    ...
    def get_thread_messages(self) -> list:
        ...
        try:
            ...
            from opscode.ui.widgets.messages import UserMessage, AssistantMessage, ToolCallMessage, MessageList
            container = self.query_one("#messages", MessageList)
            for child in container.children:
    ...
    def _reset_thread_usage(self, cost_usd: float = 0.0, context_tokens: int = 0) -> None:
        ...
        from opscode.utils.session_stats import SessionStats
    ...
    async def _write_live_approval_mode(self, mode: ApprovalMode | None = None) -> bool:
        ...
        if agent_obj is None or thread_id is None:
            return False
        from opscode.approval_mode import awrite_approval_mode
    ...
    def _prompt_yolo_switcher_acknowledgement(self) -> None:
        from opscode.approval_mode import save_yolo_acknowledgement
    ...
    def _notify_auto_mode_enabled_once(self) -> None:
        from opscode.approval_mode import ApprovalMode, has_auto_mode_notice, save_auto_mode_notice
        from opscode.ui.widgets.auto_mode_notice import AutoModeNoticeScreen
    ...
    def action_toggle_debug_console(self) -> None:
        ...
        from opscode.ui.widgets.debug_console import DebugConsoleScreen
    ...
    async def _open_text_area_in_editor(
        ...
        *,
        ...
        from opscode.editor import ExternalEditorError, open_in_editor
    ...
    def _build_debug_snapshot(self) -> list[Any]:
        ...
        from opscode._debug import installed_debug_log_path
        from opscode.config.env_vars import DEBUG, is_env_truthy
        from opscode._version import __version__
        from opscode.ui.widgets.debug_console import SnapshotField
        import logging
        from pathlib import Path
    ...
    async def _show_model_selector(self) -> None:
        ...
        from opscode.model.config import resolve_model_spec
    ...
    async def _prompt_model_auth_if_needed(self, spec: str) -> bool:
        ...
        """
        from opscode.model.config import (
            get_credential_env_var,
            get_provider_auth_status,
        )
        from opscode.ui.widgets.auth import AuthPromptScreen, AuthResult
    ...
    async def _switch_model(self, spec: str, effort: str | None = None) -> None:
        ...
        if ":" in spec:
            ...
            from opscode.model.factory import detect_provider
    ...
    async def _finalize_model_switch(self, spec: str, effort: str | None = None) -> None:
        ...
        """
        from opscode.model.config import save_recent_model, apply_stored_credentials
    ...
    async def _show_effort_selector(self) -> None:
        ...
        from opscode.model.reasoning import (
            default_effort_for_model,
            supported_efforts_for_model,
        )
        from opscode.ui.widgets.effort_selector import EffortSelectorScreen
    ...
    async def _set_effort_override(self, effort: str) -> None:
        ...
        if effort.lower() in {"clear", "reset"}:
            ...
            os.environ.pop("OPSCODE_REASONING_EFFORT", None)
            settings_obj = getattr(self, "settings", None)
            if settings_obj:
    ...
    async def _show_agent_selector(self) -> None:
        ...
        from opscode.ui.widgets.agent_selector import AgentSelectorScreen
    ...
    async def _invoke_skill(
        ...
        *,
        ...
        """
        from opscode.skills.invocation import build_skill_invocation_envelope
        from opscode.skills.loader import load_skill_content
        from opscode.ui.widgets.messages import AppMessage, ErrorMessage, SkillMessage, UserMessage
