...
import logging
import os
from opscode.config.paths import GLOBAL_ENV_PATH, ensure_data_dir, upsert_env_vars
...
from opscode.model.config import get_credential_env_var, get_provider_display_name
...
class AuthPromptScreen(ModalScreen[AuthResult]):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical():
            ...
            if self._is_langsmith:
                yield Static(
                    Content.from_markup(
                        "opscode stores the above key locally and turns on LangSmith tracing. "
                ...
                yield Static(
                    Content.from_markup(
                        "opscode stores the above key locally and uses it "
            ...
            if self._env_var:
                key_meta = Static(
                    Content.assemble(
                        ...
                        "After setting one in a .env file, restart opscode to pick "
                        "it up. ",
                        (
                            "Configuration docs",
            ...
            if self._is_langsmith:
                ...
                current_proj = os.environ.get("LANGSMITH_PROJECT") or os.environ.get("OPSCODE_LANGSMITH_PROJECT") or "opscode"
                proj_input = Input(
                    value=current_proj,
                    placeholder="LANGSMITH_PROJECT (default: opscode)",
                ...
                proj_hint = Static(
                    Content.from_markup(
                        ...
                        "Leave blank to use the default [bold]opscode[/bold]."
                    ),
                    classes="auth-prompt-meta",
    ...
    def _perform_save(self) -> None:
        ...
        if self._is_langsmith:
            ...
            proj = "opscode"
            try:
                proj = self.query_one("#auth-prompt-project-input", Input).value.strip() or "opscode"
            except Exception:
                pass
