...
them grouped by category with value source indicators. Every setting is
interactive: booleans toggle, choices cycle, strings/paths/lists open an inline
editor, credentials redirect to the auth manager, and theme/model open their
dedicated selector screens.
"""
...
class ConfigManagerScreen(ModalScreen[None]):
    """Modal screen for searching, inspecting, and modifying OpsCode configuration settings.
    ...
    def compose(self) -> ComposeResult:
        with Vertical():
            yield Static("OpsCode Configuration", classes="config-title")
            yield Input(
    ...
    def on_mount(self) -> None:
        from opscode.config.manifest import load_config_toml
    ...
    def _open_secret_editor(self, opt: ConfigOption) -> None:
        ...
        try:
            from opscode.ui.widgets.auth_manager import AuthManagerScreen
            ...
            try:
                ...
                def _on_theme_result(result: str | None) -> None:
                    if result:
                        ...
                        from opscode.ui.theme import save_theme_preference
        ...
        try:
            ...
            if key.startswith("permissions."):
                ...
                if store is not None:
                    ...
                    from opscode.ui.permission_store import _DEFAULT_ALLOW, _DEFAULT_ASK, _DEFAULT_DENY
    ...
    def _find_option(key: str) -> ConfigOption | None:
        ...
        from opscode.config.manifest import get_option
