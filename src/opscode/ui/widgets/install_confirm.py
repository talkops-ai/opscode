"""Confirmation modals for package installation in OpsCode TUI."""
...
from opscode.model.config import PROVIDER_DISPLAY_NAMES
...
class InstallProviderConfirmScreen(ModalScreen[bool]):
    ...
    def compose(self):
        ...
        if self._model_spec is not None:
            body = (
                f"To use [bold]{self._model_spec}[/bold], opscode needs to "
                f"install the [bold]{self._extra}[/bold] integration. This will add "
                f"the provider package to your opscode environment."
            )
        else:
            body = (
                f"To add a key for [bold]{provider_name}[/bold], opscode needs to "
                f"install the [bold]{self._extra}[/bold] integration. This will add "
                f"the provider package to your opscode environment."
            )
...
__all__ = ["InstallProviderConfirmScreen"]
