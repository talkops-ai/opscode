...
`opscode.*` log records sourced from the in-memory ring buffer in
...
from opscode._debug_buffer import (
    DEFAULT_CAPACITY,
    InMemoryLogRecord,
    get_log_buffer,
    retention_bucket_for_level,
)
from opscode.ui.clipboard import copy_text_to_clipboard
from opscode.ui.widgets._copy_spans import copy_span_style, copy_span_target
from opscode.ui.widgets._links import open_style_link
from opscode.security.unicode_security import sanitize_control_chars
...
def _debug_records_enabled() -> bool:
    ...
    return logging.getLogger("opscode").isEnabledFor(logging.DEBUG)
...
class DebugConsoleScreen(ModalScreen[None]):
    ...
    def _cached_langsmith_urls(self) -> dict[str, str]:
        ...
        from opscode.config.langsmith import get_cached_langsmith_thread_url
    ...
    async def _fetch_langsmith_link(self, thread_id: str) -> None:
        ...
        """
        from opscode.config.langsmith import build_langsmith_thread_url
