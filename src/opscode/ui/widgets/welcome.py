"""Welcome banner & first-run onboarding widget for OpsCode TUI.
...
def _collect_skills() -> list[tuple[str, str]]:
    ...
    try:
        ...
        built_in_dir = Path(opscode.__file__).parent / "built_in_skills"
...
def _collect_subagents() -> list[tuple[str, str]]:
    ...
    try:
        ...
        from opscode.subagents import get_built_in_subagents, list_subagents

        seen: dict[str, str] = {}
        for meta in get_built_in_subagents():
            seen[meta["name"]] = meta.get("description", "")
        for meta in list_subagents(
            user_agents_dir=settings.get_user_agents_dir(),
            project_agents_dir=settings.get_project_agents_dir(),
        ):
            seen[meta["name"]] = meta.get("description", "")
        effective_root = settings.effective_project_root
        if effective_root:
            project_plugins = load_project_plugins(effective_root)
            for sa in project_plugins.subagent_metas:
                seen[sa["name"]] = sa.get("description", "")
...
class WelcomeBanner(Widget):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.border_title = "OpsCode - DevOps Coding Agent by TalkOps.ai"
        self._skills_data = []
