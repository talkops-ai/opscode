"""OpsCode brand colors and semantic constants for the TUI.
...
Textual CSS variables set via `register_theme()` in `OpsCodeApp.__init__`.
...
# ---------------------------------------------------------------------------
# Brand palette — dark (Tokyo Night base + OpsCode teal identity)
# ---------------------------------------------------------------------------
DC_DARK = "#11121D"
...
DC_TEAL = "#2dd4bf"
"""Primary accent teal — OpsCode identity."""
...
def _load_user_themes() -> dict[str, ThemeEntry]:
    """Load user-defined themes from ~/.opscode/config.toml [themes.<name>] sections."""
...
def register_app_themes(app: App[Any]) -> None:
    """Register custom OpsCode and user-defined themes with Textual."""
    ...
    try:
        ...
        logger.warning("Failed registering default opscode themes", exc_info=True)

    reg = get_registry()
    for name, entry in reg.items():
        if entry.custom and name not in ("opscode-dark", "opscode-light"):
            try:
                t = Theme(
                    name=name,
...
def load_theme_preference() -> str:
    ...
    env_theme = os.environ.get("OPSCODE_THEME", "").strip()
...
def save_theme_preference(name: str, config_path: Path | None = None) -> bool:
    """Save global theme preference to ~/.opscode/config.toml."""
    from opscode.config.toml_config import save_theme_preference as _save
    return _save(name, config_path=config_path or CONFIG_PATH)


def save_terminal_theme_mapping(term_program: str, name: str) -> bool:
    """Save per-terminal default theme to ~/.opscode/config.toml."""
...
def get_theme_colors(widget_or_app: object = None) -> ThemeColors:
    ...
    For custom themes (OpsCode-branded and user-defined), the pre-built
    ...
    # Custom themes (OpsCode-branded / user-defined) use pre-built colors —
