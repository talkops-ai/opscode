"""Global settings, environment detection, and bootstrap for opscode."""
...
def resolve_env_var(name: str) -> str | None:
    """Resolve env var with OPSCODE_ prefix priority.
    
    1. Check OPSCODE_{name} (takes precedence)
    2. Check plain {name}
...
def _load_dotenv(*, start_path: Path | None = None, refresh_loaded: bool = False) -> None:
    """Load .env files: project-level (walk-up), then ~/.opscode/.env.
    ...
    # 1. Global ~/.opscode/.env (individual provider keys live here)
...
class Settings:
    ...
    @property
    def user_opscode_dir(self) -> Path:
