...
Implements the `coding-agent-v1` contract for OpsCode: identity block, plugin/runtime
...
from opscode.config.env_vars import EXPERIMENTAL, USER_ID, is_env_truthy
...
CODING_AGENT_INTEGRATION = "opscode"
CODING_AGENT_RUNTIME = "OpsCode"
...
def build_stream_config(
    ...
    *,
    ...
    if is_env_truthy(EXPERIMENTAL):
        metadata["opscode_experimental"] = True

    if auto_approve:
        metadata["opscode_auto_approve"] = True
    ...
    if deepagents_version is not None:
        metadata["opscode_client_deepagents_version"] = deepagents_version

    metadata["opscode_term_program"] = os.environ.get("OPSCODE_TERM_PROGRAM") or os.environ.get(
        "TERM_PROGRAM", "vscode"
    )
    ...
    if assistant_id:
        metadata.update(
            {
                "opscode_agent_name": assistant_id,
