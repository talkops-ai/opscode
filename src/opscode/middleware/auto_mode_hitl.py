...
_ASYNC_APPROVAL_ROUTING_KEY = "_opscode_async_approval_routing"
...
class AsyncApprovalHITLMiddleware(HumanInTheLoopMiddleware[Any, Any, Any]):
    ...
    async def aafter_model(
        ...
        from opscode.security.approval_mode_source import _aresolve_approval_mode
...
from opscode.middleware.goal_state_notice import project_goal_state
from opscode.ui.widgets._ask_user_types import (
...
USER_PROMPT_METADATA_KEY = "opscode_user_prompt"
...
def is_mcp_tool(tool: object) -> bool:
    """Return whether a tool carries opscode's MCP wrapper marker."""
    metadata = getattr(tool, "metadata", None)
...
_CLASSIFIER_POLICY = (
    "You are opscode's action authorization classifier.\n"
    ...
    "opscode successfully allocated during this request.\n\n"
...
class AutoModeHITLMiddleware(HumanInTheLoopMiddleware[AutoModeState, Any, Any]):
    ...
    def __init__(
        ...
        *,
        ...
        from opscode.utils.git import read_git_remote_url_from_filesystem
        ...
        def create_temp_artifact(
            ...
            file, such as a pull-request body passed with `--body-file`. OpsCode chooses
    ...
    def _managed_temp_rejection(self, request: ToolCallRequest) -> ToolMessage | None:
        ...
        if trusted_tool is not None and request.tool is not trusted_tool:
            return ToolMessage(
                content=(
                    "Denied a tool-name collision with opscode's managed temporary "
                    "artifact tools."
                ),
                name=tool_name,
    ...
    async def _classify(
        ...
        invoke = structured.ainvoke(
            messages,
            config={
                "run_name": "opscode_auto_classifier",
                "tags": ["opscode:auto"],
    ...
    async def aafter_model(
        ...
        from opscode.middleware.server_hooks import hook_decided_permission
