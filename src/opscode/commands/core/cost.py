"""Cost / token usage command handler for OpsCode."""
...
class CostHandler(BaseCommandHandler):
    ...
    async def execute(self, ctx: CommandContext) -> CommandResult:
        ...
        if ctx.app is not None:
            ...
            # 2. Retrieve approximate conversation token count
            ...
            if conv_tokens is None and hasattr(ctx.app, "query"):
                try:
                    from opscode.ui.widgets.messages import AssistantMessage, UserMessage, ToolCallMessage
                    acc = 0
