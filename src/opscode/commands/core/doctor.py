"""Diagnostic health-check command handler for OpsCode.
...
class DoctorHandler(BaseCommandHandler):
    ...
    def _collect_diagnostics(self, ctx: CommandContext) -> DiagnosticSection:
        ...
        items: list[DiagnosticItem] = [
            DiagnosticItem("opscode", __version__),
            DiagnosticItem(
                "Python",
    ...
    def _collect_active_model(self, ctx: CommandContext) -> DiagnosticSection:
        ...
        if context_limit:
            from opscode.model.config import format_token_count
    ...
    def _collect_configuration(self, ctx: CommandContext) -> DiagnosticSection:
        ...
        from opscode.config.paths import classify_path, PathState
    ...
    def _collect_agents(self, ctx: CommandContext) -> DiagnosticSection:
        ...
        from opscode.config.paths import user_agent_md, DEFAULT_AGENT_NAME
    ...
    def _detect_editable_install() -> bool:
        """Check if opscode is installed in editable/dev mode."""
        try:
            from importlib.metadata import distribution
            dist = distribution("opscode")
    ...
    def _get_commit_hash() -> str | None:
        """Get the git commit hash for the opscode install."""
        git = shutil.which("git")
        if not git:
            return None
        import subprocess
