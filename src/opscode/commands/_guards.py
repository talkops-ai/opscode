...
if TYPE_CHECKING:
    from opscode.commands._base import BaseCommandHandler, CommandContext
...
async def require_confirmation(handler: BaseCommandHandler, ctx: CommandContext) -> bool:
    ...
    Uses the existing ApprovalModalScreen from opscode.ui.approval.
    ...
    if ctx.app is not None and hasattr(ctx.app, "push_screen_wait"):
        try:
            from opscode.ui.widgets.approval import ApprovalModalScreen
