"""Runtime command handlers for opscode (/reload, /restart, /install, /update, /auto-update)."""
...
class UpdateHandler(BaseCommandHandler):
    ...
    async def execute(self, ctx: CommandContext) -> CommandResult:
        ...
        try:
            from opscode import __version__ as current_version
        except ImportError:
...
def _check_pypi_version(*, prerelease: bool = False) -> str | None:
    """Query PyPI for the latest opscode version."""
    try:
        import json
        import urllib.request

        url = "https://pypi.org/pypi/opscode/json"
...
def _perform_upgrade(
    *, target_version: str, prerelease: bool = False
    ...
    """Run pip install to upgrade opscode."""
    cmd = ["pip", "install", "--upgrade", f"opscode=={target_version}"]
    if prerelease:
        cmd.append("--pre")
