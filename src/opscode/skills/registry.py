"""Skill registry to discover, validate, and index opscode skills."""
...
class SkillRegistry:
    ...
    def get_sources_for_middleware(self) -> list[tuple[str, ...]]:
        ...
        try:
            from opscode.plugins import discover_marketplace_plugins
            from opscode.plugins.project_plugins import _has_agents, load_project_plugins
        ...
        user_opscode_dir = settings.get_user_skills_dir()
        if user_opscode_dir:
            sources.append((str(user_opscode_dir), "User OpsCode"))
        ...
        if effective_project_root:
            psd = effective_project_root / ".opscode" / "skills"
            if psd.is_dir():
                sources.append((str(psd), "Project OpsCode"))
