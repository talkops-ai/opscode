...
Precedence for memory locations:
  ...
  1. **Project-scoped** (highest priority): `{project_root}/.opscode/memory/`
  2. **User-scoped** (fallback): `~/.opscode/memory/`
...
class MemoryStore:
    ...
    def __init__(
        ...
        *,
        ...
        self._project_dir = (
            (project_root / ".opscode" / "memory") if project_root else None
        )
        self._user_dir = (user_home or Path.home()) / ".opscode" / "memory"
