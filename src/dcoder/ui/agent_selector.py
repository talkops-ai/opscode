...
Modal dialog matching dcode's ``AgentSelectorScreen`` — lists agents
discovered from ``~/.dcoder/``, supports ↑/↓/Tab navigation, Enter to
select, Ctrl+S to toggle default, and Esc to cancel.

On selection returns the agent name string.  The caller
(``DCoderApp._show_agent_selector``) handles the actual agent switch.
"""
...
from dcoder.agent.config import (
    clear_default_agent,
    save_default_agent,
)
...
class AgentSelectorScreen(ModalScreen[str | None]):
    ...
    Displays agents found in ``~/.dcoder/`` in an ``OptionList``.
    ...
    def compose(self):
        with Vertical():
            ...
            if self._agent_names:
                ...
                yield Static(
                    "No agents found in ~/.dcoder/.\n"
                    "Run dcoder with -a <name> to create one.",
    ...
    async def action_set_default(self) -> None:
        ...
        ol = self._option_list()
        if ol is None or not self._agent_names:
            return
        ...
        if name == self._default_agent:
            ...
            try:
                ...
                logger.exception("save_default_agent raised unexpectedly")
                ok = False
        ...
        if not ok:
            help_widget.update("Failed to save default")
