"""Permission store — central permission state management for DCoder.
...
Persistence is handled via ``~/.dcoder/config.toml`` under the ``[permissions]``
...
def load_permission_store() -> PermissionStore:
    """Load a PermissionStore from ``~/.dcoder/config.toml``."""
...
def save_permission_store(store: PermissionStore) -> bool:
    """Persist a PermissionStore to ``~/.dcoder/config.toml``.
    ...
    """
    try:
        from dcoder.ui.theme import _read_config_toml_data, _write_config_toml_data
