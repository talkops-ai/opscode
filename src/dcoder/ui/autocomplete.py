...
#
# The inline autocomplete popup is managed by ChatInput's own
# MultiCompletionManager.  This stub widget only exists so
# ``DCoderApp`` can declare an ``@on(AutocompletePopup.CommandSelected)``
# handler that fires when the user picks a command from the popup.


class AutocompletePopup:
    """Stub namespace for the ``CommandSelected`` message.

    Not a real Textual widget — ChatInput manages the visual popup itself.
    This class only exposes the ``CommandSelected`` message type so
    ``DCoderApp`` can listen for it with Textual's ``@on`` decorator.
    """
