"""Canonical registry of `DCODER_CODE_*` environment variables.

Every env var the app reads whose name starts with `DCODER_CODE_` must
...
like `"DCODER_CODE_FOO"` appears in source code instead of a constant
...
!!! note
    ...
    and provider credentials: setting `DCODER_CODE_{NAME}` takes priority
    over `{NAME}`.  For example, `DCODER_CODE_OPENAI_API_KEY` overrides
...
DEBUG_FILE = "DCODER_CODE_DEBUG_FILE"
...
DEBUG_NOTIFICATIONS = "DCODER_CODE_DEBUG_NOTIFICATIONS"
...
Does not auto-open the update modal (use `DCODER_CODE_DEBUG_UPDATE` for that).

Any non-empty value enables the flag (including `"0"` or `"false"`).
"""

DEBUG_ONBOARDING = "DCODER_CODE_DEBUG_ONBOARDING"
"""Force the onboarding flow to open on every interactive startup.

Parsed by `is_env_truthy`: accepts `1`, `true`, `yes`, `on` as enabled.
"""

DEBUG_UPDATE = "DCODER_CODE_DEBUG_UPDATE"
...
DISABLED_PROJECT_MCP_SERVERS = "DCODER_CODE_DISABLED_PROJECT_MCP_SERVERS"
...
launch env, or global `~/.dcoder/.env` can.
"""

ENABLED_PROJECT_MCP_SERVERS = "DCODER_CODE_ENABLED_PROJECT_MCP_SERVERS"
...
EXTERNAL_EVENT_SOCKET = "DCODER_CODE_EXTERNAL_EVENT_SOCKET"
...
EXTERNAL_EVENT_SOCKET_PATH = "DCODER_CODE_EXTERNAL_EVENT_SOCKET_PATH"
...
dcoder project *and* one extra project via LangSmith write replicas.
...
NO_TERMINAL_ESCAPE = "DCODER_CODE_NO_TERMINAL_ESCAPE"
"""Disable all terminal escape/control sequence output when enabled."""

NO_UPDATE_CHECK = "DCODER_CODE_NO_UPDATE_CHECK"
"""Disable automatic update checking when set."""

OFFLINE = "DCODER_CODE_OFFLINE"
...
OLLAMA_DISCOVERY = "DCODER_CODE_OLLAMA_DISCOVERY"
...
ONBOARDING_INTEGRATIONS_SCREEN = "DCODER_CODE_ONBOARDING_INTEGRATIONS_SCREEN"
...
RESTARTED_AFTER_UPDATE = "DCODER_CODE_RESTARTED_AFTER_UPDATE"
...
RIPGREP_INSTALLER = "DCODER_CODE_RIPGREP_INSTALLER"
...
`~/.dcoder/bin` (no sudo). `system` skips that download so power users can
...
SHOW_SCROLLBAR = "DCODER_CODE_SHOW_SCROLLBAR"
...
SHOW_URL_OPEN_TOAST = "DCODER_CODE_SHOW_URL_OPEN_TOAST"
...
SPLASH_SHOW_CWD = "DCODER_CODE_SPLASH_SHOW_CWD"
...
SPLASH_SHOW_MODEL = "DCODER_CODE_SPLASH_SHOW_MODEL"
...
SUPPRESS_ENV_OVERRIDE_WARNING = "DCODER_CODE_SUPPRESS_ENV_OVERRIDE_WARNING"
"""Silence the startup warning emitted when a `DCODER_CODE_`-prefixed
LangSmith variable overrides its canonical counterpart (e.g. both
`LANGSMITH_API_KEY` and `DCODER_CODE_LANGSMITH_API_KEY` are set to
different values).
...
def is_env_truthy(name: str, *, default: bool = False) -> bool:
    ...
    Args:
        name: Environment variable name (typically a `DCODER_CODE_*`
