"""Centralized filesystem-path definitions for dcoder.

This module is the **single source of truth** for every directory and file
dcoder reads or writes at runtime.  It is intentionally dependency-free (no
...
Architecture: mirrors the ``deepagents_code._paths`` + ``_constants`` pattern
from the dcode reference codebase.
"""
...
ENV_PREFIX: Final[str] = "DCODER_"
"""All dcoder-specific env vars use this prefix."""
...
DATA_DIR: Final[Path] = Path.home() / ".dcoder"
"""``~/.dcoder/`` — Top-level user data directory."""

STATE_DIR: Final[Path] = DATA_DIR / ".state"
"""``~/.dcoder/.state/`` — Machine-managed state (never hand-edit)."""
...
def agent_dir(name: str = DEFAULT_AGENT_NAME) -> Path:
    ...
    The default agent directory is ``~/.dcoder/dcoder/``.
    """
    return DATA_DIR / name


def user_skills_dir(name: str = DEFAULT_AGENT_NAME) -> Path:
    """Return ``~/.dcoder/{name}/skills/``."""
    return agent_dir(name) / "skills"


def user_agents_dir(name: str = DEFAULT_AGENT_NAME) -> Path:
    """Return ``~/.dcoder/{name}/agents/``."""
    return agent_dir(name) / "agents"


def user_agent_md(name: str = DEFAULT_AGENT_NAME) -> Path:
    """Return ``~/.dcoder/{name}/AGENTS.md``."""
    return agent_dir(name) / "AGENTS.md"


# ── Project directory helpers ────────────────────────────

def project_dcoder_dir(project_root: Path) -> Path:
    """Return ``{project_root}/.dcoder/``."""
    return project_root / ".dcoder"


def project_skills_dir(project_root: Path) -> Path:
    """Return ``{project_root}/.dcoder/skills/``."""
    return project_dcoder_dir(project_root) / "skills"


def project_agents_dir(project_root: Path) -> Path:
    """Return ``{project_root}/.dcoder/agents/``."""
    return project_dcoder_dir(project_root) / "agents"
...
def project_mcp_paths(project_root: Path) -> list[Path]:
    ...
    Precedence (highest → lowest):
      ...
      3. ``{project_root}/.dcoder/.mcp.json``
      4. ``{project_root}/.dcoder/mcp.json``
    """
    return [
        project_root / ".mcp.json",
        project_root / "mcp.json",
        project_dcoder_dir(project_root) / ".mcp.json",
        project_dcoder_dir(project_root) / "mcp.json",
    ]
...
def project_agent_md_paths(project_root: Path) -> list[Path]:
    ...
    """
    return [
        project_dcoder_dir(project_root) / "AGENTS.md",
...
def ensure_data_dir() -> Path:
    """Create ``~/.dcoder/`` if it doesn't exist."""
    DATA_DIR.mkdir(parents=True, exist_ok=True)
    return DATA_DIR


def ensure_state_dir() -> Path:
    """Create ``~/.dcoder/.state/`` if it doesn't exist."""
    STATE_DIR.mkdir(parents=True, exist_ok=True)
    return STATE_DIR


def ensure_agent_dir(name: str = DEFAULT_AGENT_NAME) -> Path:
    """Create ``~/.dcoder/{name}/`` if it doesn't exist."""
    d = agent_dir(name)
...
def ensure_plugins_dir() -> Path:
    """Create ``~/.dcoder/plugins/`` if it doesn't exist."""
    PLUGINS_DIR.mkdir(parents=True, exist_ok=True)
    return PLUGINS_DIR


def ensure_conversation_history_dir() -> Path:
    """Create ``~/.dcoder/conversation_history/`` if it doesn't exist."""
