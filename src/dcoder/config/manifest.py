...
``resolve_scalar`` is the shared resolution engine: env var (with ``DCODER_``
prefix priority) → ``config.toml`` → typed default, returning both the
effective value and its source label.

Architecture: mirrors the ``deepagents_code.config_manifest`` pattern from the
dcode reference codebase.
"""
...
def resolve_scalar(
    ...
    *,
    ...
    # 2. Environment variable (with DCODER_ prefix priority)
    if option.env_var:
...
_STATIC_OPTIONS: tuple[ConfigOption, ...] = (
    ...
    ConfigOption(
        ...
        kind=OptionKind.STR,
        env_var="DCODER_EXTRA_SKILLS_DIRS",
