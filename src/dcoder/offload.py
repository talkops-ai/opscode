"""Storage paths for offloaded conversation history in DCoder."""
...
from dcoder.config.paths import DATA_DIR

logger = logging.getLogger(__name__)

_FALLBACK_ARTIFACTS_ROOT = "/dcoder-artifacts-fallback"
...
def _artifacts_root() -> _ArtifactsStorage:
    ...
    root = temp_root / f"dcoder-artifacts-{suffix}"
    try:
        _harden_dir(root)
        ...
        unique = Path(
            tempfile.mkdtemp(prefix=f"dcoder-artifacts-{suffix}-", dir=temp_root)
        )
        _harden_dir(unique)
...
def _offload_fallback_root() -> Path:
    ...
    suffix = str(getuid()) if getuid is not None else str(os.getpid())
    temp_root = Path(tempfile.gettempdir())
    path = temp_root / f"dcoder-{suffix}"
    try:
        ...
        unique = Path(tempfile.mkdtemp(prefix=f"dcoder-{suffix}-", dir=temp_root))
