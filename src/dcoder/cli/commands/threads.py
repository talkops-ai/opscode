...
from dcoder.state.session import (
    delete_thread,
...
async def list_threads_command(
    ...
    *,
    ...
    """CLI handler for `dcoder threads list`."""
    ...
    if not threads:
        ...
        console.print("[dim]Start a conversation with: dcoder[/dim]")
        return
...
async def delete_thread_command(
    ...
    *,
    ...
    """CLI handler for `dcoder threads delete <thread_id>`."""
    if dry_run:
        if output_format == "json":
