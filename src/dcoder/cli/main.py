...
from dcoder.output import OutputFormat, add_json_output_arg
...
def parse_args() -> argparse.Namespace:
    ...
    from dcoder.cli.commands.auth import setup_auth_parser
    from dcoder.cli.commands.config import setup_config_parser
    from dcoder.cli.commands.mcp import setup_mcp_parsers
    from dcoder.plugins.commands_cli import setup_plugin_parser
    from dcoder.skills.commands import setup_skills_parser
    ...
    parser = argparse.ArgumentParser(
        prog="dcoder",
        description="DCoder — AI Coding & DevOps Assistant",
...
def cli_main() -> None:
    ...
    try:
        ...
        if command == "help":
            from dcoder.ui.ui_help import show_help

            show_help()
            sys.exit(0)

        if command == "config":
            from dcoder.cli.commands.config import run_config_command

            sys.exit(run_config_command(args))

        if command == "auth":
            from dcoder.cli.commands.auth import run_auth_command

            sys.exit(run_auth_command(args))

        if command == "doctor":
            from dcoder.cli.commands.doctor import run_doctor_command

            sys.exit(run_doctor_command(args))

        if command == "tools":
            from dcoder.cli.commands.tools import run_tools_command

            sys.exit(run_tools_command(args))

        if command in {"plugin", "plugins"}:
            from dcoder.plugins.commands_cli import execute_plugin_command

            sys.exit(execute_plugin_command(args))

        if command == "skills":
            from dcoder.skills.commands import execute_skills_command

            sys.exit(execute_skills_command(args))

        if command == "threads":
            from dcoder.cli.commands.threads import delete_thread_command, list_threads_command
        ...
        if command == "agents":
            from dcoder.cli.commands.agents import list_agents, reset_agent
        ...
        if command == "mcp":
            from dcoder.cli.commands.mcp import run_mcp_config, run_mcp_list, run_mcp_login
        ...
        if getattr(args, "default_model", None) is not None:
            from dcoder.model.config import load_default_model, save_default_model
        ...
        assistant_id = getattr(args, "agent", None) or "dcoder"
