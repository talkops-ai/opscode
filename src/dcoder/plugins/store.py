...
def load_marketplace_records(
    *, project_root: Path | None = None, strict: bool = False
    ...
    if target_root is None:
        try:
            cwd = Path.cwd()
            if (cwd / ".dcoder").is_dir() or (cwd / ".dcoder-plugin").is_dir() or (cwd / "marketplace.json").is_file():
                target_root = cwd
    ...
    if target_root is not None:
        try:
            ...
            dcoder_dir = target_root / ".dcoder"
            manifest_path = find_marketplace_manifest(dcoder_dir) or find_marketplace_manifest(target_root)
...
def load_all_disabled_plugin_ids(
    ...
    if isinstance(effective_root, Path):
        from dcoder.config.paths import project_local_settings_path, project_settings_path
...
def _user_settings_path() -> Path:
    from dcoder.config.paths import USER_SETTINGS_PATH
    return USER_SETTINGS_PATH


def load_user_enabled_plugin_ids() -> frozenset[str]:
    """Read enabledPlugins from ``~/.dcoder/settings.json``."""
...
def load_project_enabled_plugin_ids(project_root: Path) -> frozenset[str]:
    """Read enabledPlugins from ``{project}/.dcoder/settings.json``."""
    if not isinstance(project_root, Path):
        return frozenset()
    from dcoder.config.paths import project_settings_path
...
def load_local_enabled_plugin_ids(project_root: Path) -> frozenset[str]:
    """Read enabledPlugins from ``{project}/.dcoder/settings.local.json``."""
    if not isinstance(project_root, Path):
        return frozenset()
    from dcoder.config.paths import project_local_settings_path
...
def load_enabled_plugin_ids(*, strict: bool = False) -> frozenset[str]:
    ...
    Reads from ``~/.dcoder/settings.json``.  For full merged
...
def set_plugin_enabled_for_scope(
    ...
    from dcoder.config.paths import (
