...
async def test_cost_handler_zero_tokens():
    ...
    assert res.message is not None and res.message is not None and "No token usage yet" in res.message
...
async def test_cost_handler_with_usage_and_limit():
    ...
    res = await handler.execute(ctx)
    assert res.success is True
    assert res.message is not None and res.message is not None and "32.0k / 128.0k tokens (25%)" in res.message
