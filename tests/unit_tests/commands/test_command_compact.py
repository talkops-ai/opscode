...
async def test_compact_requires_minimum_messages():
    ...
    assert res.message is not None and "Not enough messages" in res.message
...
async def test_compact_agent_running_error():
    ...
    res = await handler.execute(ctx)
    assert res.success is False
    assert res.message is not None and "Cannot compact while agent is running" in res.message
...
async def test_compact_success_token_savings():
    ...
    assert res.message is not None and "Conversation Compacted" in res.message
    assert res.message is not None and "Freed:" in res.message
