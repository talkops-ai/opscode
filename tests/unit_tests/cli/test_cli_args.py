...
Verifies that ``opscode`` CLI flags are parsed correctly and that mutually
...
from opscode.cli.main import build_version_text, parse_args, _validate_args
...
class TestParseArgs:
    ...
    def test_non_interactive_with_dash_n(self):
        ...
        with patch("sys.argv", ["opscode", "-n", "summarize README"]):
            args = parse_args()
...
class TestBuildVersionText:
    ...
    def test_contains_opscode(self):
        text = build_version_text()
        assert "opscode" in text

    def test_has_version_number(self):
        text = build_version_text()
        # Should have at least "opscode X.Y.Z" pattern
        parts = text.split()
        assert len(parts) >= 2
