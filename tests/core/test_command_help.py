...
async def test_general_help_returns_categorized_output():
    ...
    assert res.message is not None and "Interactive Features:" in res.message
...
async def test_specific_help_known_command():
    ...
    res = await handler.execute(ctx)
    assert res.success is True
    assert res.message is not None and "**Command:** `/testcore`" in res.message
    assert res.message is not None and "**Category:** Core" in res.message
...
async def test_specific_help_unknown_command():
    ...
    assert res.message is not None and "Unknown command: `/unknown`" in res.message
