...
def test_dotenv_precedence(tmp_path, monkeypatch):
    ...
    # Mock config manifest's DEFAULT_CONFIG_DIR to point to our temp global dir
    import dcoder.config.settings as config_mod
    monkeypatch.setattr(config_mod, "DEFAULT_CONFIG_DIR", global_dir)
