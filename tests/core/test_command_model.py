...
async def test_model_direct_switch():
    ...
    res = await handler.execute(ctx)
    assert res.success is True
    assert "anthropic:claude-3-5-sonnet" in res.message
...
async def test_model_set_default():
    ...
    res = await handler.execute(ctx)
    assert res.success is True
    assert "Default model set to: `gpt-4o`" in res.message
...
async def test_model_clear_default():
    ...
    res = await handler.execute(ctx)
    assert res.success is True
    assert "Default model cleared." in res.message
...
def test_get_model_profile_and_token_format():
    ...
    assert prof["name"] == "Kimi K3"
    assert prof["max_input_tokens"] == 1_000_000
    assert prof["reasoning_output"] is True
    assert prof["tool_calling"] is True
