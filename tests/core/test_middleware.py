...
def test_middleware_registry():
    ...
    registry.register("middleware_b", MiddlewareB, order=20)
    registry.register("middleware_a", MiddlewareA, order=10)

    stack = registry.build_stack()
    assert len(stack) == 2
    assert isinstance(stack[0], MiddlewareA)
    assert isinstance(stack[1], MiddlewareB)
