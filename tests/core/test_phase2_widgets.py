...
def test_autocomplete_filtering():
    """Verify AutocompletePopup filtering logic."""
    popup = AutocompletePopup()
    popup.filter_query("/pl")
    filtered = popup._filtered_entries
    assert any(entry.name == "/plan" for entry in filtered)

    popup.filter_query("/help")
    filtered = popup._filtered_entries
    assert len(filtered) == 1
    assert filtered[0].name == "/help"
