...
async def test_skills_handler_non_interactive_text():
    ...
    assert "User Skills" in res.message
    assert "aws-ecs" in res.message
    assert "Location:" in res.message
    assert "Purpose:" in res.message
...
async def test_skills_handler_empty_state():
    ...
    assert ".dcoder/skills/" in res.message
...
async def test_skills_handler_categorization():
    ...
    res = await handler.execute(ctx)
    assert res.success
    assert "Project Skills" in res.message
    assert "User Skills" in res.message
    assert "Plugin Skills" in res.message
    assert "Built-in Skills" in res.message
