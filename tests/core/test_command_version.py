...
async def test_version_handler_output():
    ...
    assert res.message is not None and "dcoder-code version:" in res.message
    assert res.message is not None and "Python version:" in res.message
    assert "claude-3-5-sonnet" in res.message
