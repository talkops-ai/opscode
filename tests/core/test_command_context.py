...
async def test_context_handler_token_and_resource_display():
    ...
    assert "25,000 / 100,000 tokens (25.0%)" in res.message
    assert "2 tools · 1 MCP servers · 1 skills" in res.message
...
async def test_context_handler_infrastructure_context():
    ...
    res = await handler.execute(ctx)
    assert res.success is True
    assert "Infrastructure Context:" in res.message
    assert "aws:us-east-1" in res.message
    assert "prod-cluster" in res.message
